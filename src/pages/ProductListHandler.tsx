...
function ProductListHandler() {
  ...
  const handleSaveProductList = async (productListName: string) => {
    try {
      const response = await fetch('http://localhost:3000/productslists', {
        method: "POST",
  ...
  const handleDeleteProductsList = async (id: number) => {
    try {
  ...
  if (isLoading) {
    return <div>Loading product lists...</div>;
  }
    ...
    <div className="mt-8">
          ...
          {productlists.map((productlist) =>  (
              ...
              <div className="flex justify-between">
                ...
                <button onClick={() => handleDeleteProductsList(productlist.id)}><TrashIcon className="w-8 h-6  hover:text-red-500 "></TrashIcon></button>
                </div>
